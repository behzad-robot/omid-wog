...
export default class SiteBuildsRouter extends SiteRouter
{
    ...
    {
        super(siteModules);
        this.router.get('/game/:slug', (req, res) =>
        {
            siteModules.Cache.getGame({ slug: req.params.slug },(err,game)=>
            {
                if(err)
                {
                ...
                siteModules.Build.find({ gameId: game._id, limit: 30 }).then((builds) =>
                {
                    siteModules.Champion.find({ gameId: game._id }).then((champions) =>
                    {
                        //now connect champions to builds:
