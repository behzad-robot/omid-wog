export const ESL_TEAMS = [
    {
        token: 'vici_gaming',
        name: 'Vici Gaming',
        logo: '/storage/esl-one-birmingham-2019/teams/vici_gaming.jpg',
        players: [
            {
                id: 'Paparazi',
                name: 'Zhang Chengjun',
                position: '1,2'
            },
            {
                id: 'Ori',
                name: 'Zeng Jiaoyang',
                position: '1,2'
            },
            {
                id: 'Yang',
                name: 'Zhou Haiyang',
                position: '3'
            },
            {
                id: 'Fade',
                name: 'Pan Yi',
                position: '4'
            },
            {
                id: 'Dy',
                name: 'Ding Cong',
                position: '5'
            },
        ]
    },
    {
        token: 'gambit_esports',
        name: 'Gambit Esports',
        logo: '/storage/esl-one-birmingham-2019/teams/gambit.jpg',
        players: [
            {
                id: 'Daxak',
                name: 'Nikita Kuzmin',
                position: '1,2'
            },
            {
                id: 'Afoninje',
                name: 'Andrey Afonin',
                position: '1,2'
            },
            {
                id: ' AfterLife',
                name: 'Vasily Shishkin',
                position: '3'
            },
            {
                id: 'Immersion',
                name: 'Alexander Hmelevskoy',
                position: '4'
            },
            {
                id: 'fng',
                name: 'Artiom Barshack',
                position: '5'
            },
        ]
    },
    {
        token: 'ninjas_in_pyjamas',
        name: 'Ninjas in Pyjamas',
        logo: '/storage/esl-one-birmingham-2019/teams/ninjas-in-pajamas.jpg',
        players: [
            {
                id: 'Ace',
                name: 'Marcus Hoelgaard',
                position: '1'
            },
            {
                id: 'Fata',
                name: 'Adrian Trinks',
                position: '2'
            },
            {
                id: '33',
                name: 'Neta Shapira',
                position: '3'
            },
            {
                id: 'Saksa',
                name: 'Martin Sazdov',
                position: '4'
            },
            {
                id: 'ppd',
                name: 'Peter Dager',
                position: '5'
            },
        ]
    },
    {
        token: 'psg_lgd',
        name: 'PSG.LGD',
        logo: '/storage/esl-one-birmingham-2019/teams/lgd.jpg',
        players: [
            {
                id: 'Ame',
                name: 'Wang Chunyu',
                position: '1'
            },
            {
                id: 'Somnus丶M',
                name: 'Lu Yao',
                position: '2'
            },
            {
                id: 'Chalice',
                name: 'Yang Shenyi',
                position: '3'
            },
            {
                id: 'fy',
                name: 'Xu Linsen',
                position: '4'
            },
            {
                id: 'xNova',
                name: 'Yap Jian Wei',
                position: '5'
            },
        ]
    },
    {
        token: 'og',
        name: 'OG',
        logo: '/storage/esl-one-birmingham-2019/teams/og.jpg',
        players: [
            {
                id: 'ana',
                name: 'Anathan Pham',
                position: '1,2'
            },
            {
                id: 'Topson',
                name: 'Topias Taavitsainen',
                position: '1,2'
            },
            {
                id: '7ckngMad',
                name: 'Sebastien Debs',
                position: '3'
            },
            {
                id: 'JerAx',
                name: 'Jesse Vainikka',
                position: '4'
            },
            {
                id: 'N0tail',
                name: 'Johan Sundstein',
                position: '5'
            },
        ]
    },
    {
        token: 'team_liquid',
        name: 'Team Liquid',
        logo: '/storage/esl-one-birmingham-2019/teams/team-liquid.jpg',
        players: [
            {
                id: 'MATUMBAMAN',
                name: 'Lasse Urpalainen',
                position: '1,2'
            },
            {
                id: 'Miracle-',
                name: 'Amer Al-Barkawi',
                position: '1,2'
            },
            {
                id: 'ChMinD_ContRoLalice',
                name: 'Ivan Ivanov',
                position: '3'
            },
            {
                id: 'GH',
                name: 'Maroun Merhej',
                position: '4'
            },
            {
                id: 'KuroKy',
                name: 'Kuro Salehi Takhasomi',
                position: '5'
            },
        ]
    },
    {
        token: 'evil_geniuses',
        name: 'Evil Geniuses',
        logo: '/storage/esl-one-birmingham-2019/teams/evil_genious.jpg',
        players: [
            {
                id: 'Arteezy',
                name: 'Artour Babaev',
                position: '1'
            },
            {
                id: 'SumaiL',
                name: 'Sumail Hassan',
                position: '2'
            },
            {
                id: 's4',
                name: 'Gustav Magnusson',
                position: '3'
            },
            {
                id: 'Cr1t-',
                name: 'Andreas Nielsen',
                position: '4'
            },
            {
                id: 'Fly',
                name: 'Tal Aizik',
                position: '5'
            },
        ]
    },
    {
        token: 'team_secret',
        name: 'Team Secret',
        logo: '/storage/esl-one-birmingham-2019/teams/team-secret.jpg',
        players: [
            {
                id: 'Nisha',
                name: 'Michal Jankowski',
                position: '1,2'
            },
            {
                id: 'MidOne',
                name: 'Yeik Nai Zheng',
                position: '1,2'
            },
            {
                id: 'zai',
                name: 'Ludwig Wåhlberg',
                position: '3'
            },
            {
                id: 'YapzOr',
                name: 'Yazied Jaradat',
                position: '4'
            },
            {
                id: 'Puppey',
                name: 'Clement Ivanov',
                position: '5'
            },
        ]
    },
    {
        token: 'alliance',
        name: 'Alliance',
        logo: '/storage/esl-one-birmingham-2019/teams/alliance.jpg',
        players: [
            {
                id: ' Madara',
                name: 'Omar Dabachach',
                position: '1 (Sub)'
            },
            {
                id: 'miCKe',
                name: 'Michael Vu',
                position: '2'
            },
            {
                id: ' Boxi',
                name: 'Samuel Svahn',
                position: '3'
            },
            {
                id: 'iNSaNiA',
                name: 'Aydin Sarkohi',
                position: '4,5'
            },
            {
                id: 'Taiga',
                name: 'Tommy Le',
                position: '4,5'
            },
        ]
    },
    {
        token: 'forward_gaming',
        name: 'Forward Gaming',
        logo: '/storage/esl-one-birmingham-2019/teams/forward-gaming.jpg',
        players: [
            {
                id: 'YawaR',
                name: 'Yawar Hassan',
                position: '1'
            },
            {
                id: 'CCnC',
                name: 'Quinn Callahan',
                position: '2'
            },
            {
                id: 'Sneyking',
                name: 'Jingjun Wu',
                position: '3 (Trial)'
            },
            {
                id: 'MSS',
                name: 'Arif Anwar',
                position: '4'
            },
            {
                id: 'pieliedie',
                name: 'Johan Åström',
                position: '5'
            },
        ]
    },
    {
        token : 'keen_gaming',
        name : 'Keen Gaming',
        logo : '/storage/esl-one-birmingham-2019/teams/keen-gaming.jpg',
        players : [
            {
                id : 'old chicken',
                name : 'Wang Zhiyong',
                position : '1'
            },
            {
                id : '-',
                name : 'Zhai Jingkai',
                position : '2'
            },
            {
                id : 'eLeVeN',
                name : 'Ren Yangwei',
                position : '3'
            },
            {
                id : 'Kaka',
                name : 'Hu Liangzhi',
                position : '4'
            },
            {
                id : 'dark ',
                name : 'Song Runxi',
                position : '5'
            },
        ]
    },
    {
        token : 'tnc_predator',
        name : 'TNC Predator',
        logo : '/storage/esl-one-birmingham-2019/teams/tnc.jpg',
        players : [
            {
                id : 'Gabbi',
                name : 'Kim Villafuerte',
                position : '1'
            },
            {
                id : 'Armel',
                name : 'Armel Paul',
                position : '2'
            },
            {
                id : 'Kuku',
                name : 'Carlo Palad',
                position : '3'
            },
            {
                id : 'Tims',
                name : 'Timothy Randrup',
                position : '4/5'
            },
            {
                id : 'eyyou',
                name : 'Nico Barcelon',
                position : '4/5'
            },
        ]
    }
];
export const GROUP_A = ['vici_gaming', 'team_liquid', 'ninjas_in_pyjamas', 'og', 'forward_gaming', 'tnc_predator'];
export const GROUP_B = ['team_secret', 'evil_geniuses', 'psg_lgd', 'keen_gaming', 'alliance', 'gambit_esports'];

